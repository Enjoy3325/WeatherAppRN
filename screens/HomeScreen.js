                ...
                loading ? (
                    ...
                    <SafeAreaView className="flex flex-1">
                        <View style={{ height: '7%' }} className="mx-4 relative z-50">
                            <View className="flex-row justify-end rounded-full"
                                style={{ backgroundColor: isToggleSearch ? theme.bgWhite(0.175) : 'transparent' }}>
                                {
                                    isToggleSearch ? (
                                        <TextInput
                                            onChangeText={handleTextDebounce}
                                            placeholder='Search city' placeholderTextColor={'#FFF8DC'} className="px-6 h-10 flex-1 text-2xl text-white" />
                                    ) : null
                                }

                                <TouchableOpacity onPress={() => setIsToggleSearch(!isToggleSearch)}
                        ...
                        <View className="flex-1 flex justify-around mx-4 mb-2">
                            ...
                            <View className="flex-row flex justify-center">
                                <Image source={weatherData[current?.condition?.text]}
                                    className="w-52 h-52" />
                            </View>
                            {/* Degree celcius */}
                                        ...
                                        weather?.forecast?.forecastday?.map((item, index) => {
                                            ...
                                            return (
                                                <View key={index} className="flex justify-center items-center w-24 rounded-3xl py-3 space-y-1 mr-4"
                                                    ...
                                                    <Image source={weatherData[item?.day?.condition?.text]} className=" h-11 w-11" />
